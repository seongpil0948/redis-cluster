...
class RedisClusterPoller:
    ...
    def run_polling_cycle(self, cycle: int) -> PollingResult:
        ...
        for key, value in test_data.items():
            try:
                ...
                if node_info not in shard_results:
                    shard_results[node_info] = {"set_count": 0, "get_count": 0, "errors": 0}
        ...
        print(f"✅ Success: {success_count}/{total_operations} ({success_rate:.1f}%) | "
    ...
    def save_results(self, results: List[PollingResult]):
        ...
        if output_data["summary"]["total_operations"] > 0:
            output_data["summary"]["overall_success_rate"] = round(
                (output_data["summary"]["total_successes"] / output_data["summary"]["total_operations"]) * 100, 2
            )
    ...
    def run(self, duration_seconds: Optional[int] = None):
        ...
        try:
            ...
            print(f"   Total operations: {self.total_stats['total_successes'] + self.total_stats['total_errors']}")
            print(f"   Successful operations: {self.total_stats['total_successes']}")
            print(f"   Failed operations: {self.total_stats['total_errors']}")
            
            if self.total_stats['total_successes'] + self.total_stats['total_errors'] > 0:
                success_rate = (self.total_stats['total_successes'] / 
                              (self.total_stats['total_successes'] + self.total_stats['total_errors'])) * 100
...
def main():
    ...
    parser.add_argument("--keys", type=int, default=50, 
                       help="테스트할 키 개수 (기본값: 50)")
    parser.add_argument("--duration", type=int, 
                       help="테스트 지속시간(초). 미지정시 무한 실행")
    parser.add_argument("--env", choices=["local", "dev", "prd"], default="local",
